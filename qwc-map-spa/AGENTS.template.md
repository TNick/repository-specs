...
graph before changing the map SPA, QWC vendor tree, overrides, bundler
config, or map automation hooks.

Treat the QWC (or equivalent) vendor directory as read-only. Put
customizations in the application-owned overrides tree that mirrors upstream
paths. Do not edit submodule sources in place.
